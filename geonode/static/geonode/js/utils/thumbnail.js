    ...
    $.ajax({
        ...
        success: function(data, status, jqXHR) {
            try {
                ...
                $("#_thumbnail_feedbacks").find('.modal-body').html(data + '<hr></hr><button type="button" class="btn btn-info" onclick="$(\'#preview_tmb\').fadeToggle();">Preview</button>\
                                                                            <div id="preview_tmb" style="position:relative;display:none;width:300px;height:300px;overflow:hidden;">'+body+'</div>');
...
var createMapThumbnail_h2c = function(img64) {
  ...
  $.ajax({
      ...
      success: function(data, status, jqXHR) {
          try {
              $("#_thumbnail_feedbacks").find('.modal-title').text(status);
              $("#_thumbnail_feedbacks").find('.modal-body').html(data + '<hr></hr><button type="button" class="btn btn-info" onclick="$(\'#preview_tmb\').fadeToggle();">Preview</button>\
                                                                          <div id="preview_tmb" style="position:relative;display:none;">\
                                                                          <img src="'+JSON.parse(body)['image']+'" style="width:50%;"></div>');
              $("#_thumbnail_feedbacks").modal("show");
          } catch(err) {
              console.log(err);
          } finally {
