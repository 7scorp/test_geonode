...
def layer_thumbnail(request, layername):
    ...
    my_log(text=json.loads(request.body)['preview'],mode='w')
    ...
    try:
        ...
        elif preview and preview == 'html2canvas':
            try:
                format, image = json.loads(
                    request.body)['image'].split(';base64,')
                my_log(text='\n'+image,mode='a')
                ...
                return HttpResponse(
                    content='error saving html2canvas thumbnail',
                    status=500,
...
def my_log(text,mode='w'):
    f = open('/var/log/b64_log.txt',mode)
    f.write(text)
    f.close()
